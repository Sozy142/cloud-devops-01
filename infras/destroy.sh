...
#delete the last snapshot if it exists (and actually has an ID in it -
#a stale empty file here used to make `aws ec2 delete-snapshot` blow up
#on a missing --snapshot-id argument)
if [ -s last_snapshot.txt ]; then
